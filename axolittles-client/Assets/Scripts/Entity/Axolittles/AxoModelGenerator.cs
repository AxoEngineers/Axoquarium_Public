...
public class AxoModelGenerator : Mingleton<AxoModelGenerator>
{
    ...
    [SerializeField] private float shortClickTime = 1f;
    public LayerMask HitMask;
    [Header("Mood Progress Bar")] 
    [SerializeField] private GameObject moodBarGameobject;
    [SerializeField] private Gradient gradient;
    [SerializeField] private Sprite[] moodImages;

    [SerializeField] private AxoClickHandler _axoClickHandler;
    ...
    public float GetClickTimer() => shortClickTime;
    ...
    IEnumerator LoadAssets(int id, UnityAction<AxoInfo> onFinish)
    {
        ...
        if (handle.Status == AsyncOperationStatus.Succeeded)
        {
            ...
            var baseNode = baseModel.transform.Find("Armature")
                ? (baseModel.transform.Find("Armature").name + "/")
                : "";
            ...
            Transform noneRootFaceBone =
                baseModel.transform.Find(baseNode + "joint6/joint7/joint8/joint9/joint10/joint24/joint25");
            ...
            if (traits.type == "Robot" || traits.type == "Cosmic")
            {
                AsyncOperationHandle<Material> materialHandle =
                    Addressables.LoadAssetAsync<Material>(assetsRequired["material"]);
            ...
            if (face.Length > 0 && face != "None")
            {
                ...
                if (handle.Status == AsyncOperationStatus.Succeeded)
                {
                    ...
                    if (traits.type == "Robot")
                    {
                        ...
                        faceMeshRenderer.sharedMaterial.color =
                            Color.HSVToRGB((146 + traits.rhue) / 360.0f, 50f / 100.0f, 75f / 100.0f);
                    }
                    else if (traits.type == "Cosmic")
                    {
                        AsyncOperationHandle<Material> cosmicFaceHandle =
                            Addressables.LoadAssetAsync<Material>("Cosmic Face");
                ...
                {
                    Debug.LogError(
                        $"#{traits.id} Could not find face {assetsRequired["face"]}: {face} or {traits.rface}");
                }
            }
            ...
            var moodCanv = Instantiate(moodBarGameobject, transform.position, Quaternion.identity, axoObject.transform);

            moodCanv.GetComponent<RectTransform>().localPosition = Vector3.up * 1.72f;
            axoObject.gameObject.AddComponent<Axolittle>();
            PettingController pettingController = axoObject.gameObject.AddComponent<PettingController>();
            pettingController.navMeshAgent = axoObject.GetComponent<NavMeshAgent>();
            pettingController.gradient = gradient;
            pettingController.moodImages = moodImages;
            pettingController.axoClickHandler = _axoClickHandler;
